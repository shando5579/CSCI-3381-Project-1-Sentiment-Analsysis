...
public class TweetCollection 
{
	...
	public int prediction(Tweet tweet)
	{
		...
		Iterator<String> iterP = positives.iterator();
		while (iterP.hasNext())
		{
			if (tweet.getContents().toLowerCase().contains(iterP.next()))
			{
				points += 1;
			}
		}
			
		Iterator<String> iterN = negatives.iterator();
		while (iterN.hasNext())
		{
			if (tweet.getContents().toLowerCase().contains(iterN.next()))
			{
				points -= 1;
			}
		}
